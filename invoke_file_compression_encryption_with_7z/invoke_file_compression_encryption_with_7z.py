...
class CustomFormatter(argparse.RawTextHelpFormatter,
    ...
    """A custom formatter class for Argument Parser to print default 
    arguments with help strings"""
    pass
...
def verify(params):
    """Verifies if the file was properly encrypted by getting 7z to list files again"""
    if not os.path.isfile(params['dest_path']):
        print("[-] Path: {dest_path} not found".format(**params))
        sys.exit(1)
    else:
        subprocess.call(shlex.split("{7z_path} l \"{dest_path}\"".format(**params)))

def main():
    """Function contains the main code to call when script is executed"""
    parser = argparse.ArgumentParser(description="Compresses and/or encrypts a file via 7z installed on system",
    ...
    parser.add_argument("-p", "--password", dest="password", help="Password to use to encrypt the file. If no encryption wanted, simply exclude this argument", action="store")
    ...
    parser.add_argument("-ve", "--verify", dest="verify", action="store_true", help="Verify if file was properly encrypted by listing files via '7z l' - user may be prompted to supply a password to verify")
    ...
    if params['verify']:
        print("[*] Verifying if file: {dest_path} was properly encrypted".format(**params))
        verify(params)
